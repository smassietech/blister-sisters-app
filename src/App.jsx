...
// 🛠️ SETTINGS
// ==========================================
const DEBUG_FORCE_RACE_DAY = false; // Set to true to preview June 6th dashboard
...
export default function App() {
  ...
  // Auth Listener (Fixed the hang issue here)
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => { 
      setUser(currentUser); 
      // If there is no user, we stop loading immediately to show the AuthScreen
  ...
  useEffect(() => {
    ...
    const unsubPlan = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'training', 'plan'), async (s) => {
      ...
      setLoading(false); // Stop loading once plan is ready
    });
  ...
  useEffect(() => {
    ...
    setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'profiles', user.uid), { 
      totalMiles: totalM, totalRuns: totalR, lastActive: new Date().toISOString()
    }, { merge: true });
  ...
  if (loading) return <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center text-pink-500 font-sans font-black uppercase tracking-widest"><Watch className="animate-spin w-12 h-12 mb-6" /> Powering Up...</div>;
...
function DashboardView({ logs, openLogModal, todayWorkout, totalMiles, completionPct, profile, currentWeekNum, diffDays }) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 shadow-lg shadow-pink-950/10">
        <div className="flex justify-between items-end mb-3"><h3 className="text-xs font-bold text-neutral-400 uppercase tracking-widest">Plan Completion</h3><span className="text-xl font-black text-white">{Number(completionPct)}%</span></div>
        <div className="w-full bg-neutral-950 rounded-full h-3 border border-neutral-800 overflow-hidden relative">
          <div className="bg-gradient-to-r from-pink-600 to-teal-400 h-full transition-all duration-1000" style={{ width: `${completionPct}%` }}></div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-neutral-900 rounded-2xl p-4 border border-neutral-800 flex flex-col justify-between shadow-xl"><Activity className="text-pink-500 w-5 h-5 mb-1" /><div className="text-2xl font-black text-white">{(Number(totalMiles)||0).toFixed(1)}</div><div className="text-[10px] text-neutral-500 font-bold uppercase tracking-wider">Miles Logged</div></div>
        <div className="bg-neutral-900 rounded-2xl p-4 border border-neutral-800 flex flex-col justify-between shadow-xl"><CalendarDays className="text-pink-400 w-5 h-5 mb-1" /><div className="text-2xl font-black text-white">{Number(timeLeft.days)}d</div><div className="text-[10px] text-neutral-500 font-bold uppercase tracking-wider">Until Race Day</div></div>
      </div>
      <div>
        <h3 className="text-sm uppercase font-black text-pink-500 mb-4 flex items-center"><Flame className="w-4 h-4 mr-2" /> Up Next</h3>
        {todayWorkout ? (
          <div onClick={() => openLogModal(todayWorkout.day, todayWorkout.week)} className="group cursor-pointer bg-neutral-900 border border-pink-500/30 hover:border-pink-500/60 rounded-2xl p-5 transition-all relative overflow-hidden shadow-2xl">
            <p className="text-[10px] font-bold bg-pink-500/20 text-pink-400 px-3 py-1 rounded-full uppercase mb-3 inline-block">Week {Number(todayWorkout.week.week)} • {String(todayWorkout.day.day)}</p>
            <p className="text-lg font-medium text-white leading-relaxed">{String(todayWorkout.day.workout)}</p>
            <div className="mt-4 flex items-center text-[10px] text-neutral-500 uppercase font-black tracking-widest"><Activity className="w-4 h-4 mr-1.5" /> Tap to log</div>
          </div>
        ) : <p className="text-neutral-500 italic text-sm text-center py-4 bg-neutral-900 rounded-2xl border border-neutral-800">Rest day! Enjoy it, sister.</p>}
      </div>
    </div>
  );
}
...
function RaceDayDashboard({ raceMeta, laps, user, db, appId, currentProfile }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-pink-600 to-rose-700 rounded-3xl p-8 text-center shadow-2xl relative overflow-hidden"><div className="relative z-10"><p className="text-[10px] font-black uppercase tracking-widest text-pink-200 mb-2">Race Time Elapsed</p><h2 className="text-5xl md:text-6xl font-black text-white tracking-tighter tabular-nums">{elapsed}</h2></div></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-neutral-900 border border-pink-500/30 rounded-3xl p-6 shadow-xl">
          <h3 className="text-xs font-black uppercase tracking-widest text-pink-500 mb-4">On Course</h3>
          {runningLap ? <div className="flex items-center space-x-4"><div className="text-2xl">🏃‍♀️</div><div><p className="text-xl font-black text-white">{String(runningLap.runnerName)}</p><p className="text-xs text-neutral-500">Lap {String(runningLap.lapNumber)}</p></div></div> : <p className="text-neutral-500 italic text-sm">Nobody out right now.</p>}
        </div>
        <div className="bg-neutral-900 border border-teal-500/30 rounded-3xl p-6 shadow-xl">
          ...
          {nextLap ? <div className="flex items-center space-x-4"><div className="text-2xl">🔥</div><div><p className="text-xl font-black text-white">{String(nextLap.runnerName)}</p><p className="text-xs text-neutral-500">Ready for Lap {String(nextLap.lapNumber)}</p></div></div> : <p className="text-neutral-500 italic text-sm">Next slot open!</p>}
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={<Activity />} label="Team Miles" val={totalMiles} unit="mi" color="text-pink-500" />
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
        <div className="bg-neutral-900 rounded-3xl border border-neutral-800 p-5 md:p-6 overflow-hidden shadow-xl">
                  ...
                  return (
                    <div key={day.id} className={`p-4 flex gap-4 transition-colors ${isRest ? 'opacity-50' : 'hover:bg-neutral-800/40'}`}>
                      ...
                      <div className="flex-1" onClick={() => !isEditMode && !isRest && openLogModal(day, week)}>
                        ...
                        {isCompleted && logData && (<div className="mt-3 p-3 bg-neutral-900 border border-teal-500/20 rounded-xl shadow-lg"><div className="flex flex-wrap gap-y-2 items-center space-x-4 text-xs font-medium text-teal-400">{logData.distance > 0 && <span>{logData.distance} mi</span>}{logData.duration > 0 && <span>{logData.duration} min</span>}{logData.vibe && <span>{String(logData.vibe)}</span>}</div></div>)}
                      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-neutral-900 border border-pink-500/30 rounded-3xl p-8 max-w-sm w-full shadow-2xl relative">
        ...
        <div className="text-center mb-8 relative z-10"><div className={`w-20 h-20 rounded-full bg-gradient-to-br ${avatarBg} flex items-center justify-center text-4xl shadow-2xl mx-auto mb-4 border-2 border-white/20`}>{String(avatarEmoji)}</div><h2 className="text-2xl font-black text-white">{existingProfile ? 'Edit Profile' : 'Welcome, Sister.'}</h2></div>
        <form onSubmit={handleSave} className="space-y-6 text-left relative z-10">
          ...
          <div><label className="text-[10px] font-black uppercase text-neutral-500 tracking-widest ml-1 mb-2 block">Icon</label><div className="grid grid-cols-6 gap-2">{AVATAR_EMOJIS_LIST.slice(0,18).map(emoji => (<button key={emoji} type="button" onClick={() => setAvatarEmoji(emoji)} className={`w-10 h-10 text-xl flex items-center justify-center rounded-lg transition-all ${avatarEmoji === emoji ? 'bg-pink-600 border border-pink-400 scale-110' : 'bg-neutral-950 border border-neutral-800'}`}>{emoji}</button>))}</div></div>
          <div><label className="text-[10px] font-black uppercase text-neutral-500 tracking-widest ml-1 mb-2 block">Back Color</label><div className="grid grid-cols-6 gap-2">{AVATAR_BGS_LIST.map(bg => (<button key={bg} type="button" onClick={() => setAvatarBg(bg)} className={`w-10 h-10 rounded-full bg-gradient-to-br ${bg} transition-all ${avatarBg === bg ? 'ring-2 ring-white scale-110 shadow-lg' : 'opacity-60'}`} />))}</div></div>
          <button type="submit" disabled={saving} className="w-full bg-pink-600 text-white font-black uppercase py-4 rounded-xl mt-4 shadow-[0_0_20px_rgba(219,39,119,0.3)]">{saving ? 'Saving...' : 'Lock it in'}</button>
        </form>
      </div>
    </div>
  );
      ...
      {activeTab === 'leaderboard' && (
        <div className="bg-neutral-900 rounded-3xl border border-neutral-800 overflow-hidden divide-y divide-neutral-800/50">
          ...
          {sortedProfiles.map((p, i) => (
            <div key={p.id} className="p-4 px-6 flex items-center justify-between"><div className="flex items-center space-x-4"><span className="text-neutral-600 font-black">#{i+1}</span><div className={`w-10 h-10 rounded-full bg-gradient-to-br ${p.avatarBg || 'from-pink-500 to-teal-400'} flex items-center justify-center shadow-lg border border-white/10`}>{String(p.avatarEmoji || '?')}</div><div><p className="font-bold text-white text-sm">{String(p.displayName || 'Anon')}</p><p className="text-[10px] text-neutral-500 uppercase font-black">{Number(p.totalRuns || 0)} sessions</p></div></div><p className="text-pink-400 font-black">{(Number(p.totalMiles || 0)).toFixed(1)} mi</p></div>
          ))}
        </div>
      )}
      {activeTab === 'relay' && (
        <div className="space-y-4">
          <button onClick={createLap} className="w-full bg-neutral-800 hover:bg-neutral-700 text-white py-4 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center shadow-xl"><PlusCircle className="w-4 h-4 mr-2" /> Add Next 5-Mile Lap</button>
          {relayLaps.map((lap) => (
            <div key={lap.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
              <div className="flex items-center space-x-4"><div className="w-10 h-10 bg-neutral-950 rounded-xl flex items-center justify-center font-black border border-white/5">L{String(lap.lapNumber)}</div><div><p className="font-bold text-white">{String(lap.runnerName || "Unclaimed")}</p><p className="text-[10px] uppercase text-neutral-500 font-bold">{String(lap.status)}</p></div></div>
              <div className="flex space-x-2">
                {!lap.runnerId && <button onClick={() => claimLap(lap.id)} className="bg-teal-500/20 text-teal-400 px-4 py-2 rounded-xl text-[10px] font-black uppercase border border-teal-500/20">Claim</button>}
                {lap.runnerId === user?.uid && lap.status === 'claimed' && <button onClick={() => startLap(lap.id)} className="bg-pink-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Start</button>}
                {lap.runnerId === user?.uid && lap.status === 'running' && <button onClick={() => finishLap(lap.id)} className="bg-teal-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Finish</button>}
              </div>
            </div>
          ))}
        </div>
...
function StatCard({ icon, label, val, unit, color }) {
  return (
    <div className="bg-neutral-900 rounded-2xl p-4 border border-neutral-800 flex flex-col justify-between shadow-xl">
      <div className={`${color} mb-1`}>{React.cloneElement(icon, { className: "w-5 h-5" })}</div>
      <div><div className="text-2xl font-black text-white leading-tight">{String(val)}</div><div className="text-[10px] text-neutral-500 font-bold uppercase tracking-wider leading-none mt-1">{String(label)} <br/> {unit && <span className="opacity-50 lowercase font-normal italic">({unit})</span>}</div></div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-pink-900/40 to-neutral-900 border border-pink-500/30 rounded-3xl p-5 shadow-xl"><Activity className="text-pink-400 mb-2 w-6 h-6" /><div className="text-4xl font-black text-white">{stats.miles.toFixed(1)}</div><div className="text-[10px] text-pink-200 uppercase font-bold mt-1">Total Miles</div></div>
        <div className="bg-gradient-to-br from-teal-900/30 to-neutral-900 border border-teal-500/30 rounded-3xl p-5 shadow-xl"><Watch className="text-teal-400 mb-2 w-6 h-6" /><div className="text-4xl font-black text-white">{Math.floor(stats.time/60)}h</div><div className="text-[10px] text-teal-200 uppercase font-bold mt-1">Time on Feet</div></div>
        <div className="col-span-2 md:col-span-1 bg-gradient-to-br from-indigo-900/30 to-neutral-900 border border-indigo-500/30 rounded-3xl p-5 shadow-xl"><Mountain className="text-indigo-400 mb-2 w-6 h-6" /><div className="text-4xl font-black text-white">{stats.elev}</div><div className="text-[10px] text-indigo-200 uppercase font-bold mt-1">Elevation (ft)</div></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-md bg-neutral-900 rounded-t-3xl md:rounded-3xl border border-neutral-800 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-t border-neutral-800 flex gap-3 pb-safe"><button type="submit" form="log-form" disabled={saving} className="flex-1 bg-pink-600 hover:bg-pink-500 text-white font-black text-xs uppercase tracking-widest py-4 rounded-xl shadow-lg transition-all">{saving ? 'Saving...' : 'Save Run'}</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
      <div 
        ...
        style={{ backgroundImage: `url('https://images.unsplash.com/photo-1552674605-15c371123a61?auto=format&fit=crop&w=1200&q=80')` }}
      />
      <div className="bg-neutral-900/80 backdrop-blur-md border border-pink-500/20 rounded-3xl p-8 max-w-sm w-full shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8 relative z-10"><div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-lg shadow-pink-500/20 mb-4"><Flame className="w-8 h-8 text-white" /></div><h1 className="text-2xl font-black text-white uppercase italic tracking-tight">Blister Sisters</h1><p className="text-[10px] text-pink-400 font-black uppercase tracking-widest mt-1">Sisterhood Secure Login</p></div>
        {error && (<div className="bg-red-500/10 border border-red-500/50 text-red-400 text-[10px] font-bold p-3 rounded-xl mb-6 relative z-10">{error}</div>)}
        <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
          {!isLogin && (<div><label className="text-[10px] font-black uppercase text-neutral-500 tracking-widest ml-1 mb-1 block">Team Invite Code</label><input type="text" value={inviteCode} onChange={(e) => setInviteCode(e.target.value.toUpperCase())} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-pink-500" required /></div>)}
          <div><label className="text-[10px] font-black uppercase text-neutral-500 tracking-widest ml-1 mb-1 block">Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-pink-500" required /></div>
          <div><label className="text-[10px] font-black uppercase text-neutral-500 tracking-widest ml-1 mb-1 block">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-pink-500" required /></div>
          <button type="submit" disabled={loading} className="w-full bg-pink-600 hover:bg-pink-500 text-white font-black uppercase tracking-widest py-4 rounded-xl shadow-lg mt-4 transition-all">{loading ? 'Working...' : (isLogin ? 'Login' : 'Sign Up')}</button>
        </form>
        <button onClick={() => setIsLogin(!isLogin)} className="w-full mt-6 text-[10px] text-neutral-500 font-black uppercase tracking-widest hover:text-white transition-colors relative z-10">{isLogin ? "Join the team" : "Back to login"}</button>
      </div>
    </div>
  );
}
