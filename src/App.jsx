    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-pink-500 selection:text-white pb-24 md:pb-0">
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-pink-400 font-black tracking-widest uppercase mt-1">{isRaceDay ? "RACE DAY LIVE" : "Training"}</p>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto p-4 py-8">
        ...
        {view === 'plan' && (
          isRaceDay 
          ? <RaceDayPlanView 
              relayLaps={relayLaps} user={user} db={db} appId={appId} currentProfile={profile} profiles={teamProfiles}
        ...
        {view === 'stats' && <StatsView logs={logs} trainingPlan={trainingPlan} />}
...
function RelayBoard({ relayLaps, user, db, appId, currentProfile, profiles = [] }) {
  const [editingLapId, setEditingLapId] = useState(null);

  const claimLap = async (lapId) => {
    if (!user || !currentProfile) return;
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId), {
      runnerId: user.uid, runnerName: String(currentProfile.displayName || "Unknown Sister"), status: 'claimed'
    });
  };

  const startLap = async (lapId) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId), {
      status: 'running', startTime: Date.now()
    });
  };

  const finishLap = async (lapId) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId), {
      status: 'complete', endTime: Date.now()
    });
  };

  const unclaimLap = async (lapId) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId), {
      runnerId: null, runnerName: null, status: 'open', startTime: null, endTime: null
    });
  };

  const deleteLap = async (lapId) => {
    if (window.confirm("Remove this lap completely?")) {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId));
    }
  };
  ...
  const createLap = async () => {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'laps', lapId), {
      lapNumber: relayLaps.length + 1, runnerId: null, runnerName: null, status: 'open', createdAt: Date.now()
    });
  };

  const assignLap = async (lapId, assigneeId) => {
    if (assigneeId === 'unclaim') {
      await unclaimLap(lapId);
  ...
  return (
    <div className="space-y-4">
      ...
      {relayLaps.map((lap) => (
        <div key={lap.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between shadow-xl gap-4">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-neutral-950 rounded-xl flex items-center justify-center font-black border border-white/5 shrink-0">L{String(lap.lapNumber)}</div>
            <div>
              <p className="font-bold text-white">{String(lap.runnerName || "Unclaimed")}</p>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            {editingLapId === lap.id ? (
              <div className="flex items-center space-x-2 w-full md:w-auto">
                <select 
                  className="bg-neutral-950 border border-neutral-700 text-white text-[10px] uppercase font-bold px-3 py-2 rounded-xl focus:border-pink-500 focus:outline-none flex-1 md:w-48"
                  onChange={(e) => assignLap(lap.id, e.target.value)}
                  defaultValue=""
                >
                  <option value="" disabled>Assign to...</option>
                  <option value="unclaim">-- Unclaim Lap --</option>
                  {profiles.sort((a,b) => String(a.displayName || '').localeCompare(String(b.displayName || ''))).map(p => (
                    <option key={p.id} value={p.id}>{p.displayName}</option>
                  ))}
                </select>
                <button onClick={() => setEditingLapId(null)} className="bg-neutral-800 text-neutral-400 p-2 rounded-xl hover:text-white transition-colors border border-neutral-700">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
              <>
                ...
                {lap.status === 'claimed' && lap.runnerId === user?.uid && <button onClick={() => unclaimLap(lap.id)} className="bg-neutral-800 text-neutral-400 px-4 py-2 rounded-xl text-[10px] font-black uppercase border border-neutral-700 hover:text-white transition-colors">Unclaim</button>}
                {lap.status === 'claimed' && lap.runnerId === user?.uid && <button onClick={() => startLap(lap.id)} className="bg-pink-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Start</button>}
                
                {lap.status === 'running' && lap.runnerId === user?.uid && <button onClick={() => finishLap(lap.id)} className="bg-teal-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Finish</button>}
                
                {(lap.status === 'running' || lap.status === 'complete') && (
                  <button onClick={() => { if(window.confirm("Reset this lap back to Open?")) unclaimLap(lap.id); }} className="bg-neutral-800 text-neutral-400 p-2 rounded-xl border border-neutral-700 hover:text-white transition-colors" title="Reset Lap">
                    <RefreshCw className="w-4 h-4" />
                  </button>
                )}

                {(lap.status === 'open' || lap.status === 'claimed') && (
                  <button onClick={() => setEditingLapId(lap.id)} className="bg-neutral-800 text-neutral-400 px-3 py-2 rounded-xl border border-neutral-700 hover:text-white transition-colors text-[10px] font-black uppercase" title="Assign Runner">
                    Assign
                  </button>
                )}

                {lap.status === 'open' && (
                  <button onClick={() => deleteLap(lap.id)} className="bg-rose-500/10 text-rose-500 p-2 rounded-xl border border-rose-500/20 hover:bg-rose-500/20 transition-colors" title="Delete Lap">
                    <X className="w-4 h-4" />
                  </button>
                )}
              </>
...
function RaceDayPlanView({ relayLaps, user, db, appId, currentProfile, profiles, logs, trainingPlan, completedLogIds, openLogModal, getLogForDay, currentWeekNum, diffDays, onEditDay }) {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {activeTab === 'relay' && (
        <div className="animate-in fade-in duration-300">
          <RelayBoard relayLaps={relayLaps} user={user} db={db} appId={appId} currentProfile={currentProfile} profiles={profiles} />
        </div>
      )}
      ...
      {activeTab === 'archive' && (
        <PlanView 
          ...
          diffDays={diffDays} onEditDay={onEditDay} isArchive={true} 
        />
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
         <h2 className="text-2xl font-black tracking-tight">Sisterhood Roster</h2>
      ...
      {!isRaceDay && (
        <div className="flex space-x-2 bg-neutral-900 p-1.5 rounded-xl border border-neutral-800 shadow-xl mb-6">
          <button onClick={() => setActiveTab('roster')} className={`flex-1 py-2 text-xs font-bold uppercase rounded-lg ${activeTab === 'roster' ? 'bg-neutral-800 text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-300'}`}>The Squad</button>
          <button onClick={() => setActiveTab('relay')} className={`flex-1 py-2 text-xs font-bold uppercase rounded-lg ${activeTab === 'relay' ? 'bg-pink-600 text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-300'}`}>Relay Board</button>
        </div>
      )}
              ...
              <div key={p.id} className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 shadow-2xl relative overflow-hidden">
                 <div className="flex items-center space-x-4 mb-4">
                   ...
                   <div>
                     ...
                     <p className="text-[10px] text-pink-400 uppercase font-black tracking-widest mt-1">{Number(p.totalRuns || 0)} Sessions • {(Number(p.totalMiles || 0)).toFixed(1)} mi</p>
                   </div>
                 </div>
      ...
      {showSettings && (
         <RaceSettingsModal 
            raceMeta={raceMeta} db={db} appId={appId} 
...
function RaceSettingsModal({ raceMeta, db, appId, onClose }) {
...
function DashboardView({ logs, openLogModal, todayWorkout, totalMiles, completionPct, profile, currentWeekNum, diffDays, toggleStrava }) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-neutral-900 rounded-3xl p-5 border border-neutral-800 flex flex-col justify-between shadow-2xl group hover:border-pink-500/30 transition-all"><Activity className="text-pink-500 w-5 h-5 mb-2" /><div><div className="text-2xl font-black text-white">{(Number(totalMiles)||0).toFixed(1)}</div><div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-1">Miles Logged</div></div></div>
...
function PlanView({ logs, trainingPlan, completedLogIds, openLogModal, getLogForDay, currentWeekNum, diffDays, onEditDay, isArchive = false }) {
      ...
      <div className="space-y-8">
        <div className="bg-neutral-900 rounded-3xl border border-neutral-800 p-6 shadow-2xl relative overflow-hidden">
              ...
              {trainingPlan.map((week, wIndex) => (
                <div key={week.week} className={`flex flex-col gap-2 p-1.5 rounded-xl border ${getStageColor(week.stage)}`}>
                  ...
                  {week.days.map((day, dIndex) => {
                    ...
                    return <button key={day.id} onClick={() => !isRest && openLogModal(day, week)} className={cellClass} disabled={isRest} title={`${day.day}: ${String(day.workout)}`} />;
                  })}
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6 text-[9px] font-black text-neutral-500 uppercase tracking-widest pt-5 border-t border-white/5">
            <span className="flex items-center"><div className="w-3 h-3 rounded-sm bg-neutral-950 border border-neutral-800 mr-2 opacity-30"></div> Rest</span>
                  ...
                  return (
                    <div key={day.id} className={`p-5 flex gap-5 transition-colors ${isRest ? 'opacity-40' : 'hover:bg-neutral-800/40'}`}>
                      ...
                      <div className="flex-1" onClick={() => !isEditMode && !isRest && openLogModal(day, week)}>
                        <p className={`text-base font-medium leading-relaxed ${isRest ? 'text-neutral-600 italic' : 'text-neutral-100'}`}>{String(day.workout)}</p>
...
function StatsView({ logs, trainingPlan }) {
  const stats = useMemo(() => {
            ...
            <div key={key} className="p-5 px-6 flex items-center justify-between hover:bg-neutral-800/20 transition-colors">
              <div className="flex items-center space-x-5">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-neutral-500 mt-1 uppercase font-bold tracking-widest">{data.dist.toFixed(1)} miles tracked</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-black text-white tabular-nums tracking-tighter">
                  {formatPace(data.dur, data.dist)} <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">/mi</span>
                </div>
              </div>
            </div>
          ))}
...
function LogModal({ day, existingLog, onClose, db, user, appId, profile }) {
  ...
  const handleStravaSync = () => {
    setDistance('3.2'); setDuration('31'); setElevation('120'); setNotes('Imported from Strava'); setVibe('🚀'); 
  };
      ...
      <div className="bg-neutral-900 border border-white/5 rounded-[3rem] p-10 max-w-sm w-full shadow-2xl relative max-h-[95vh] overflow-y-auto">
        ...
        {existingProfile && onResetPlan && (
          <div className="mt-8 pt-6 border-t border-white/5 relative z-10 text-center">
            <button type="button" onClick={() => { onResetPlan(); onClose(); }} className="text-[10px] text-neutral-500 hover:text-pink-400 font-black uppercase tracking-[0.2em] flex items-center justify-center w-full transition-colors"><RefreshCw className="w-3 h-3 mr-2" /> Master Reset Team Plan</button>
          </div>
        )}
      </div>
    </div>
