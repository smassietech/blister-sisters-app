      ...
      <div className="space-y-8">
        <div className="bg-neutral-900 rounded-3xl border border-neutral-800 p-6 shadow-2xl relative overflow-hidden">
              ...
              {trainingPlan.map((week, wIndex) => (
                <div key={week.week} className={`flex flex-col gap-2 p-1.5 rounded-xl border ${getStageColor(week.stage)}`}>
                  ...
                  {week.days.map((day, dIndex) => {
                    ...
                    let cellClass = "w-5 h-5 rounded-md transition-all flex items-center justify-center cursor-pointer border ";
                    if (isLogged) cellClass += "bg-pink-500 border-pink-400 shadow-[0_0_12px_rgba(236,72,153,0.5)] scale-105 z-10 text-white";
                    else if (isRest) cellClass += "bg-neutral-950 border-neutral-700/60 opacity-60 cursor-default";
                    ...
                    else cellClass += "bg-neutral-800 border-white/20 hover:border-pink-500/50";
                    ...
                    return (
                      <button key={day.id} onClick={() => !isRest && openLogModal(day, week)} className={cellClass} disabled={isRest} title={`${dayNames[dIndex]}: ${String(day.workout)}`}>
                        {isEndure24 && <Flame className={`w-3.5 h-3.5 ${isLogged ? 'text-white' : 'text-orange-500 drop-shadow-[0_0_5px_rgba(249,115,22,0.8)]'}`} />}
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6 text-[9px] font-black text-neutral-400 uppercase tracking-widest pt-5 border-t border-white/5">
            <span className="flex items-center"><div className="w-3 h-3 rounded-sm bg-neutral-950 border border-neutral-700/60 mr-2 opacity-60"></div> Rest</span>
            <span className="flex items-center"><div className="w-3 h-3 rounded-sm bg-neutral-800 border border-white/20 mr-2"></div> Future</span>
                ...
                {week.days.map((day, dIndex) => {
                  ...
                  return (
                    <div key={day.id} className={`p-5 flex gap-5 transition-colors ${isRest ? 'bg-neutral-950/30' : 'hover:bg-neutral-800/40'}`}>
                      <div className="flex-shrink-0 flex flex-col items-center pt-1 w-12"><span className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${isRest ? 'text-neutral-500' : 'text-neutral-400'}`}>{dayNames[dIndex]}</span>{isCompleted ? <CheckCircle2 className="w-6 h-6 text-teal-400 drop-shadow-[0_0_8px_rgba(45,212,191,0.4)]" /> : isRest ? <div className="w-2.5 h-2.5 rounded-full bg-neutral-700 mt-2"></div> : <div className="w-6 h-6 rounded-full border-2 border-pink-500 mt-1"></div>}</div>
